...
* La classe DatabaseConnexion à pour but de gérer la connexion avec la base de données. Cette classe ouvre une connexion au service Postrgresql via l’attribut connexion_pool. L’attribut instrance limite cette connexion à une requête à la fois, ceci permettant d'éviter des conflits d’écriture entre les DAO. Procédé ainsi nous permet de gagner en performance car la connexion à la base de données reste toujours ouverte tant que l’application est utilisée, ceci évite les déconnexions et reconnexions lors de l'exécution de chaque requête. La méthode get_connection permet d'allouer la connexion à un DAO qui la rend via la méthode release_connection pour qu’elle soit de nouveau disponible pour la prochaine requête. La méthode close_all mettant fin à la connexion quand l’application est fermée.
