...
Nous avons réflechis à différentes manières de mettre en place cette fonction. Même si nous ne sommes qu'à l'étape de la réflexion, celle qui nous semble pour l'instant la plus pertinente est la suivante. Nous coefficientons chaque critère en fonction de l'importance que nous lui accordons. 35% pour la distance, 25% pour le nombre de vélo disponible, 15% pour la fiabilité historique, 15% pour la disponibilité de vélo électrique et 10% pour la tendance récente. 

Pour la distance, nous fixons uen distance maximale, 2 kilomètres par exemple et on calcule la différence entre la distance de la station et la distance maximale. Une station à 0 mètres rapporte 100 point, 50 pour une à 1 kilomètre et 0 pour une à 2 kilomètres. 

Pour le nombre de vélo disponible, une station vide n'est pas prise en compte pour les recommandations. Pour les autres nous pensions diviser le nombre de vélos disponibles par la capacité de la station. Une station avec deux vélos disponibles sur 10 semble plus intéressantes qu'une station de 40 places avec deux vélos disponibles. Le calcul pourra être le même pour les vélos électriques à noter que nous pourrions, à l'inscription sur l'application demander à l'utilisateur par quel type de vélo il est intéressé. Si les vélos électriques ne l'intéressent pas ce coefficient pourrait être réduit à 0. 
